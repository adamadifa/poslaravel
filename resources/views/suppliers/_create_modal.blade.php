...
<div id="createSupplierModal" class="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-xs {{ $errors->hasBag('default') && !old('_method') ? 'flex' : 'hidden' }} items-center justify-center p-3 sm:p-4 overflow-y-auto">
    <div class="bg-white border border-slate-200/90 rounded-2xl max-w-xl w-full shadow-2xl transition-all my-auto overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-50/60">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-bold text-base text-slate-900 tracking-tight">Tambah Pemasok Baru</h3>
                    <p class="text-[11px] text-slate-400">Lengkapi data profil vendor, kontak PIC, dan termin pembayaran</p>
                </div>
            </div>
            <button onclick="closeCreateSupplierModal()" type="button" class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition">
                <i data-lucide="x" class="w-4 h-4"></i>
            </button>
        </div>

        <form id="createSupplierForm" action="{{ route('suppliers.store') }}" method="POST" novalidate class="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            @csrf
            ...
            <div id="create_group_name">
                <div id="create_box_name" class="relative rounded-xl border {{ $errors->has('name') && !old('_method') ? 'border-rose-500 ring-2 ring-rose-500/10' : 'border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20' }} bg-white transition px-4 pt-3 pb-2">
                    ...
                    <div class="flex items-center gap-2.5">
                        <i id="create_icon_name" data-lucide="building-2" class="w-4 h-4 {{ $errors->has('name') && !old('_method') ? 'text-rose-500' : 'text-slate-400' }} shrink-0"></i>
                        <input type="text" name="name" id="create_input_name" value="{{ old('_method') ? '' : old('name') }}" required placeholder="Contoh: PT. Sumber Alfaria Distribusi" class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none">
                    </div>
                </div>
                <p id="create_error_name" class="mt-1 text-[11px] font-medium {{ $errors->has('name') && !old('_method') ? 'text-rose-500' : 'text-slate-400' }} px-1">
                    {{ $errors->has('name') && !old('_method') ? $errors->first('name') : 'Nama lengkap badan usaha atau vendor' }}
                </p>
            </div>
                ...
                <div>
                    <div class="relative rounded-xl border border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20 bg-white transition px-4 pt-3 pb-2">
                        <label class="absolute -top-2.5 left-3.5 bg-white px-1.5 text-[11px] font-bold text-slate-700">
                            Kode Pemasok (Opsional)
                        </label>
                        <div class="flex items-center gap-2.5">
                            <i data-lucide="hash" class="w-4 h-4 text-slate-400 shrink-0"></i>
                            <input type="text" name="code" value="{{ old('_method') ? '' : old('code') }}" placeholder="Auto: SUP-0001" class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none">
                        </div>
                    </div>
                    <p class="mt-1 text-[10px] text-slate-400 px-1">Kosongkan untuk otomatis dibuat sistem</p>
                </div>
                ...
                <div>
                    <div class="relative rounded-xl border border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20 bg-white transition px-4 pt-3 pb-2">
                        ...
                        <div class="flex items-center gap-2.5">
                            ...
                            <input type="text" name="contact_person" value="{{ old('_method') ? '' : old('contact_person') }}" placeholder="Nama PIC vendor" class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none">
                        </div>
                    </div>
                ...
                <div>
                    <div class="relative rounded-xl border border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20 bg-white transition px-4 pt-3 pb-2">
                        ...
                        <div class="flex items-center gap-2.5">
                            ...
                            <input type="email" name="email" value="{{ old('_method') ? '' : old('email') }}" placeholder="sales@supplier.com" class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none">
                        </div>
                    </div>
                ...
                <div>
                    <div class="relative rounded-xl border border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20 bg-white transition px-4 pt-3 pb-2">
                        ...
                        <div class="flex items-center gap-2.5">
                            ...
                            <input type="text" name="city" value="{{ old('_method') ? '' : old('city') }}" placeholder="Jakarta, Surabaya, Bandung..." class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none">
                        </div>
                    </div>
                ...
                <div>
                    <div class="relative rounded-xl border border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20 bg-white transition px-4 pt-3 pb-2">
                        ...
                        <div class="flex items-center gap-2.5">
                            ...
                            <input type="number" name="payment_term_days" value="{{ old('_method') ? '' : old('payment_term_days', '0') }}" placeholder="0 (Cash) / 30 (Tempo)" class="w-full bg-transparent border-0 p-0 text-xs font-bold text-slate-800 focus:ring-0 focus:outline-none">
                        </div>
                    </div>
            ...
            <div>
                <div class="relative rounded-xl border border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20 bg-white transition px-4 pt-3 pb-2">
                    ...
                    <div class="flex items-start gap-2.5 pt-0.5">
                        ...
                        <textarea name="address" rows="2" placeholder="Jalan, Nomor Gedung, Kompleks Pergudangan..." class="w-full bg-transparent border-0 p-0 text-xs font-medium text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none resize-none">{{ old('_method') ? '' : old('address') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
                <button type="button" onclick="closeCreateSupplierModal()" class="px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-100 transition">Batal</button>
