<!-- MODAL EDIT UNIT -->
<div id="editUnitModal" class="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-xs {{ $errors->hasBag('default') && old('_method') === 'PUT' ? 'flex' : 'hidden' }} items-center justify-center p-3 sm:p-4 overflow-y-auto">
    <div class="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl max-w-md w-full shadow-2xl transition-all my-auto overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between shrink-0 bg-slate-50/60 dark:bg-slate-800/50">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-bold text-base text-slate-900 dark:text-white tracking-tight">Edit Satuan</h3>
                    <p class="text-[11px] text-slate-400">Perbarui nama takaran, singkatan, dan status</p>
                </div>
            </div>
            <button onclick="closeEditUnitModal()" type="button" class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition">
        ...
        <form id="editUnitForm" action="{{ old('edit_form_action', '#') }}" method="POST" novalidate class="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            ...
            <div id="edit_group_name">
                <div id="edit_box_name" class="relative rounded-xl border {{ $errors->has('name') && old('_method') === 'PUT' ? 'border-rose-500 ring-2 ring-rose-500/10' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20' }} bg-white dark:bg-slate-900 transition px-4 pt-3 pb-2">
                    <label id="edit_label_name" class="absolute -top-2.5 left-3.5 bg-white dark:bg-slate-900 px-1.5 text-[11px] font-bold {{ $errors->has('name') && old('_method') === 'PUT' ? 'text-rose-500' : 'text-slate-700 dark:text-slate-300' }}">
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        <input type="text" name="name" id="edit_input_name" value="{{ old('_method') === 'PUT' ? old('name') : '' }}" required placeholder="Contoh: Karton, Lusin" class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 dark:text-white placeholder-slate-400 focus:ring-0 focus:outline-none">
                    </div>
                </div>
            ...
            <div id="edit_group_short_name">
                <div id="edit_box_short_name" class="relative rounded-xl border {{ $errors->has('short_name') && old('_method') === 'PUT' ? 'border-rose-500 ring-2 ring-rose-500/10' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20' }} bg-white dark:bg-slate-900 transition px-4 pt-3 pb-2">
                    <label id="edit_label_short_name" class="absolute -top-2.5 left-3.5 bg-white dark:bg-slate-900 px-1.5 text-[11px] font-bold {{ $errors->has('short_name') && old('_method') === 'PUT' ? 'text-rose-500' : 'text-slate-700 dark:text-slate-300' }}">
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        <input type="text" name="short_name" id="edit_input_short_name" value="{{ old('_method') === 'PUT' ? old('short_name') : '' }}" required placeholder="Contoh: krt, lsn" class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 dark:text-white placeholder-slate-400 focus:ring-0 focus:outline-none">
                    </div>
                </div>
            ...
            <!-- Status Aktif Switch -->
            <div class="flex items-center justify-between p-3 rounded-xl bg-slate-50/50 dark:bg-slate-800/40 border border-slate-200/90 dark:border-slate-700">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <span class="block text-xs font-bold text-slate-800 dark:text-white">Status Satuan Aktif</span>
