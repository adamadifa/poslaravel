<!-- MODAL CREATE UNIT -->
<div id="createUnitModal" class="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-xs {{ $errors->hasBag('default') && !old('_method') ? 'flex' : 'hidden' }} items-center justify-center p-3 sm:p-4 overflow-y-auto">
    <div class="bg-white border border-slate-200/90 rounded-2xl max-w-md w-full shadow-2xl transition-all my-auto overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-50/60">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-bold text-base text-slate-900 tracking-tight">Tambah Satuan Baru</h3>
                    <p class="text-[11px] text-slate-400">Nama takaran dan simbol singkatan unit barang</p>
                </div>
            </div>
            <button onclick="closeCreateUnitModal()" type="button" class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition">
        ...
        <form id="createUnitForm" action="{{ route('units.store') }}" method="POST" novalidate class="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            ...
            <div id="create_group_name">
                <div id="create_box_name" class="relative rounded-xl border {{ $errors->has('name') && !old('_method') ? 'border-rose-500 ring-2 ring-rose-500/10' : 'border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20' }} bg-white transition px-4 pt-3 pb-2">
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        <input type="text" name="name" id="create_input_name" value="{{ old('_method') ? '' : old('name') }}" required placeholder="Contoh: Karton, Lusin, Botol" class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none">
                    </div>
                </div>
            ...
            <div id="create_group_short_name">
                <div id="create_box_short_name" class="relative rounded-xl border {{ $errors->has('short_name') && !old('_method') ? 'border-rose-500 ring-2 ring-rose-500/10' : 'border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20' }} bg-white transition px-4 pt-3 pb-2">
                    <label id="create_label_short_name" class="absolute -top-2.5 left-3.5 bg-white px-1.5 text-[11px] font-bold {{ $errors->has('short_name') && !old('_method') ? 'text-rose-500' : 'text-slate-700' }}">
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        <input type="text" name="short_name" id="create_input_short_name" value="{{ old('_method') ? '' : old('short_name') }}" required placeholder="Contoh: krt, lsn, btl" class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none">
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
                <button type="button" onclick="closeCreateUnitModal()" class="px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-100 transition">Batal</button>
