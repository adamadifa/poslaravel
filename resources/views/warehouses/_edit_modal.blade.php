...
<div id="editWarehouseModal" class="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-xs {{ $errors->hasBag('default') && old('_method') === 'PUT' ? 'flex' : 'hidden' }} items-center justify-center p-3 sm:p-4 overflow-y-auto">
    <div class="bg-white border border-slate-200/90 rounded-2xl max-w-lg w-full shadow-2xl transition-all my-auto overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-50/60">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-bold text-base text-slate-900 tracking-tight">Edit Gudang / Cabang</h3>
                    <p class="text-[11px] text-slate-400">Perbarui informasi cabang outlet dan status gudang default</p>
                </div>
            </div>
            <button onclick="closeEditWarehouseModal()" type="button" class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition">
                <i data-lucide="x" class="w-4 h-4"></i>
            </button>
        </div>

        <form id="editWarehouseForm" action="{{ old('edit_form_action', '#') }}" method="POST" novalidate class="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            @csrf
            @method('PUT')
            <input type="hidden" name="edit_form_action" id="edit_form_action" value="{{ old('edit_form_action') }}">
            <input type="hidden" name="warehouse_id" id="edit_warehouse_id" value="{{ old('warehouse_id') }}">
            ...
            <div id="edit_group_name">
                <div id="edit_box_name" class="relative rounded-xl border {{ $errors->has('name') && old('_method') === 'PUT' ? 'border-rose-500 ring-2 ring-rose-500/10' : 'border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20' }} bg-white transition px-4 pt-3 pb-2">
                    ...
                    <div class="flex items-center gap-2.5">
                        <i id="edit_icon_name" data-lucide="warehouse" class="w-4 h-4 {{ $errors->has('name') && old('_method') === 'PUT' ? 'text-rose-500' : 'text-slate-400' }} shrink-0"></i>
                        <input type="text" name="name" id="edit_input_name" value="{{ old('_method') === 'PUT' ? old('name') : '' }}" required placeholder="Contoh: Gudang Utama / Cabang Kemang" class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none">
                    </div>
                </div>
                <p id="edit_error_name" class="mt-1 text-[11px] font-medium {{ $errors->has('name') && old('_method') === 'PUT' ? 'text-rose-500' : 'text-slate-400' }} px-1">
                    {{ $errors->has('name') && old('_method') === 'PUT' ? $errors->first('name') : 'Nama identitas lokasi fisik gudang atau toko' }}
                </p>
            </div>
                ...
                <div>
                    <div class="relative rounded-xl border border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20 bg-white transition px-4 pt-3 pb-2">
                        ...
                        <div class="flex items-center gap-2.5">
                            ...
                            <input type="text" name="code" id="edit_input_code" value="{{ old('_method') === 'PUT' ? old('code') : '' }}" placeholder="WH-001" class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none">
                        </div>
                    </div>
                    <p class="mt-1 text-[10px] text-slate-400 px-1">Kode identifikasi cabang</p>
                </div>
                ...
                <div>
                    <div class="relative rounded-xl border border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20 bg-white transition px-4 pt-3 pb-2">
                        ...
                        <div class="flex items-center gap-2.5">
                            ...
                            <input type="text" name="phone" id="edit_input_phone" value="{{ old('_method') === 'PUT' ? old('phone') : '' }}" placeholder="021-xxxxxxx" class="w-full bg-transparent border-0 p-0 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none">
                        </div>
                    </div>
            ...
            <div>
                <div class="relative rounded-xl border border-slate-200 hover:border-slate-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20 bg-white transition px-4 pt-3 pb-2">
                    ...
                    <div class="flex items-start gap-2.5 pt-0.5">
                        ...
                        <textarea name="address" id="edit_input_address" rows="2" placeholder="Jalan, Kawasan Industri / Kompleks Ruko..." class="w-full bg-transparent border-0 p-0 text-xs font-medium text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none resize-none">{{ old('_method') === 'PUT' ? old('address') : '' }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between p-3 rounded-xl bg-amber-50/70 border border-amber-200/80">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_default" id="edit_input_is_default" value="1" {{ (old('_method') === 'PUT' ? old('is_default') : false) ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
                <button type="button" onclick="closeEditWarehouseModal()" class="px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-100 transition">Batal</button>
