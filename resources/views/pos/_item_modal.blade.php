<!-- MODAL INPUT ITEM (QTY, PILIH SATUAN, MODIFIER/TOPPING, & CATATAN) -->
<div id="itemModal" class="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-xs hidden items-center justify-center p-3 sm:p-4 overflow-y-auto">
    <div class="bg-white border border-slate-200/90 rounded-2xl max-w-lg w-full shadow-2xl transition-all my-auto overflow-hidden flex flex-col max-h-[92vh]">
        
        <!-- Header -->
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/70 shrink-0">
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    ...
                    <div class="flex items-center gap-2 text-[10px] text-slate-400 mt-0.5">
                        <span id="modal_product_code" class="font-mono bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200 text-slate-600 font-semibold">PRD-0001</span>
                        <span id="modal_product_stock" class="font-bold text-slate-600">Stok: 0</span>
                    </div>
                </div>
            </div>
            <button onclick="closeModal('itemModal')" type="button" class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition shrink-0 cursor-pointer">
                <i data-lucide="x" class="w-4 h-4"></i>
            </button>
        </div>

        <form id="itemModalForm" onsubmit="handleItemModalSubmit(event)" class="flex-1 overflow-y-auto p-5 space-y-4">
            
            <!-- Satuan Jual & Qty Grid -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div class="relative rounded-xl border-2 border-brand-500 bg-white transition px-3 pt-2 pb-1.5 shadow-2xs">
                    <label class="absolute -top-2.5 left-3.5 bg-white px-1.5 text-[10px] font-bold text-brand-600">
                        Jumlah / Qty <span class="text-rose-500">*</span>
                    </label>
                    <div class="flex items-center gap-1.5">
                        <button type="button" onclick="adjustModalQty(-1)" class="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 flex items-center justify-center text-sm font-black transition cursor-pointer">-</button>
                        <input 
                            type="number" 
                            step="any" 
                            min="0.0001" 
                            ...
                            required 
                            placeholder="1" 
                            class="w-full text-center bg-transparent border-0 p-0 text-base font-black text-slate-900 font-mono-num focus:ring-0 focus:outline-none"
                        >
                        <button type="button" onclick="adjustModalQty(1)" class="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 flex items-center justify-center text-sm font-black transition cursor-pointer">+</button>
                    </div>
                </div>
            ...
            <!-- ========================================== -->
            <!-- MODIFIER & TOPPING GROUPS (F&B OPTIONS)     -->
            <!-- ========================================== -->
            <div id="modal_modifiers_wrapper" class="hidden space-y-3 pt-1">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                        <i data-lucide="sparkles" class="w-3.5 h-3.5 text-amber-500"></i>
                        <span>Opsi Racikan & Topping</span>
                    </span>
                    <span class="text-[10px] text-slate-400">Pilih varian rasa, es, gula & topping tambahan</span>
                </div>

                <div id="modal_modifiers_container" class="space-y-3">
                    <!-- Dynamic Modifier Groups rendered via JS -->
                </div>
            </div>
            ...
            <!-- Footer Action -->
            <div class="flex items-center justify-end gap-2 pt-2 border-t border-slate-100 shrink-0">
                <button type="button" onclick="closeModal('itemModal')" class="px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-100 transition cursor-pointer">Batal</button>
